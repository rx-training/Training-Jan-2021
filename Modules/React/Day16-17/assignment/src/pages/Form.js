...
export  const Form = (props) => {
   ...
   const handleChange=(e)=>{
      ...
      if(name === 'stdImg'){
         setData({
            ...data,[name] : '/img/'+ e.target.files[0].name
         })
      } else {
         setData({
            ...data,[name] : value
      ...
      if(name === 'stdState'){
         setData({...data,stdDist : "",stdState : value})
      }
      ...
      switch(name){
         case (name.match(/Name$/) || {}).input :  
            ...
            break;
         case 'pinCode' : 
            setErrors({...errors, [name] : value.length !== 6 ? "pincode must be of 6 digit" : ( isNaN(value)  ? "Only Numbers are valid" : "")})
            break;
         case "stdDob" : 
         var age = calculateAge(data.stdDob)
            setErrors({...errors, [name] : (age < 5 || age > 20 || isNaN(age)) ? "Age must be between 5 to 20" : ""})
            break;
      ...
      <>
         <form action="" onSubmit={handleSubmit}>
            ...
            <div className="card-body row gy-3 justify-content-between align-items-center">
               ...
               <StudentDetail data={{...data, errors : {...errors}}} handleChange={handleChange}/>  

               <FatherDetails data={{...data, errors : {...errors}}} handleChange={handleChange}/>

               <MotherDetails data={{...data, errors : {...errors}}} handleChange={handleChange}/>

               <EmergencyContact data={{...data, errors : {...errors}}} handleChange={handleChange}/>
               
               <ReferanceDetails data={{...data, errors : {...errors}}} handleChange={handleChange}/>

               <UploadImage stdImg={{...data, errors : {...errors}}} handleChange={handleChange}/>
