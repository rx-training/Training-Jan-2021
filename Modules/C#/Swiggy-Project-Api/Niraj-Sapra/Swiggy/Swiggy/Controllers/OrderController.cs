...
namespace Swiggy.Controllers
{
    ...
    public class OrderController : ControllerBase
    {
        ...
        IOrder Category;
        public OrderController(IOrder custo, Swiggy_ProjectContext _context)
        {
            this.Category = custo;
        ...
        [HttpGet]
        public IEnumerable<Models.Order> AddNewDataMethod()
        {
       ...
       /* [HttpPost]
        public string creates([FromBody] Order obEntity)
        {

            Order check = context.Orders.FirstOrDefault(s => s.order == obEntity.CategoryName);
            if (check != null)
...
*/
        ...
        public string Deletes([FromBody] int id)
        {
            try
            {
                var dataDelete = context.Orders.Single(s => s.OrderId == id);
                Category.Delete(dataDelete);
                return "Order is remove successfully";
            }
            catch (Exception)
            {
