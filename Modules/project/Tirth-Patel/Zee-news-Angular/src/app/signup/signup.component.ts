...
export class SignupComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private viewerService: ViewersService
    ...
    viewerService.getViewers().subscribe(ele => { this.viewers = ele});
  ...
  checkId(username:string) {
   ...
   for (const iterator of this.userids) {
  ...
  onSubmit() {
    ...
    var x = this.checkId(this.viewer.UserId)
    if (x) {
      console.log(true);
      ...
      alert('EmailId is already in Use')
    }
