...
const StationList = ({ stations, deleteStation, editStation }) => {
  ...
  for (let i = 1; i <= Math.ceil(stations.length / itemPerPage); i++) {
    pages.push(i);
  }
  ...
  var currentItems = stations.slice(indexofFirstItem, indexofLastItem);

  useEffect(() => {
    currentItems = stations.slice(indexofFirstItem, indexofLastItem);
    setCurrentPage(1);
  }, [stations]);
