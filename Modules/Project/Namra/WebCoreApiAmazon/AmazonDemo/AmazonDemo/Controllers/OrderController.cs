...
namespace AmazonDemo.Controllers
{
    ...
    public class OrderController : ControllerBase
    {
        ...
        [Authorize]
        [HttpPut("{OrderID}")]
        public string UpdateOrder(int OrderID, [FromBody] ClassOrder clsorder)
        {
            if (user.Any(s => s.UserId == clsorder.UserId))
            {
                if(product.Any(s=> s.ProductId == clsorder.ProductId))
                {
                    return order.updateOrder(OrderID, clsorder);
                }
                else
                {
                    return $"Product id is wrong, please enter an appropriate id...";
                }
            }
            else
                return $"Your user id is wrong...";
        }
    }
}
