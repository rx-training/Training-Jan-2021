import React, { useEffect, useState } from "react";
import Navbar from "../components/Navbar";
import { FaPlus } from "react-icons/fa";
import { getToken, removeUserSession } from "../../Utils/Storage";
import Loading from "../../components/Loading";
import { Modal } from "react-bootstrap";
import BrandRow from "../components/BrandsPage/BrandRow";
import BrandService from "../../services/BrandService";

export default function BrandsPage(props) {
  const [loading, setLoading] = useState(false);
  const [brands, setBrands] = useState([]);
  const [showModal, setShowModal] = useState(false);
  const [brandName, setBrandName] = useState("");
  const [oldBrandName, setOldBrandName] = useState("");
  const [error, setError] = useState("");
  const [isUpdate, setIsUpdate] = useState(false);
  const [id, setId] = useState("");

  async function getData() {
    try {
      setLoading(true);
      let res = await BrandService.getAllBrands();

      setBrands(res.data);
      setLoading(false);
    } catch (error) {
      console.error(error);
      setLoading(false);
    }
  }

  useEffect(() => {
    getData();
  }, []);

  const handleChange = (event) => {
    setBrandName(event.target.value);
  };

  const addCategory = async () => {
    if (brandName === "") {
      setError("Category name should not be empty");
      return;
    }

    if (isUpdate) {
      try {
        setLoading(true);
        await BrandService.updateBrand(
          id,
          { brandName: brandName },
          getToken()
        );
        getData();
        closeModel();
        setBrandName("");
        setLoading(false);
        setError("");
      } catch (error) {
        if (error.response.status === 401) {
          removeUserSession();
          props.history.push("/login");
        }
        setLoading(false);
      }
    } else {
      try {
        setLoading(true);
        await BrandService.addBrand({ brandName: brandName }, getToken());
        getData();
        closeModel();
        setBrandName("");
        setLoading(false);
        setError("");
      } catch (error) {
        if (error.response.status === 401) {
          removeUserSession();
          props.history.push("/login");
        }
        setLoading(false);
      }
    }
  };

  const updateBrand = (id, name) => {
    setIsUpdate(true);
    setId(id);
    setShowModal(true);
    setOldBrandName(name);
  };

  const deleteBrand = async (id) => {
    try {
      setLoading(true);
      await BrandService.deleteBrand(id, getToken());
      getData();
      setLoading(false);
    } catch (error) {
      console.error(error);
      if (error.response.status === 401) {
        removeUserSession();
        props.history.push("/login");
      } else if (error.response.status === 409) {
        alert(error.response.data);
      }
      setLoading(false);
    }
  };

  const closeModel = () => {
    setShowModal(false);
    setIsUpdate(false);
    setId("");
  };

  if (loading) {
    return (
      <>
        <Navbar />
        <Loading />
      </>
    );
  }

  return (
    <>
      <Navbar />
      <div className="">
        <div className="container py-5">
          <button
            className="btn login-btn btn-block add-category-btn mb-4"
            onClick={() => {
              setShowModal(!showModal);
            }}
          >
            <FaPlus style={{ fontSize: "13.5px" }} /> Add Brand
          </button>

          <h1 className="text-center pb-3">Brands</h1>

          <table className="table table-hover table-bordered text-center bg-white table-striped">
            <thead className="thead-dark">
              <tr>
                <th style={{ letterSpacing: "2px" }}>No.</th>
                <th style={{ letterSpacing: "2px" }}>Brand</th>
                <th></th>
                <th></th>
              </tr>
            </thead>
            <tbody>
              {brands.map((brand, index) => {
                return (
                  <BrandRow
                    brand={brand}
                    index={index}
                    key={brand._id}
                    deleteBrand={deleteBrand}
                    updateBrand={updateBrand}
                  />
                );
              })}
            </tbody>
          </table>
        </div>
      </div>

      {/* ------------------------------- ADD BRAND MODAL ------------------------------- */}
      <Modal show={showModal} size="lg" onHide={closeModel}>
        <Modal.Header className="bg-pink text-white border">
          <Modal.Title>{isUpdate ? "Update" : "Add"} Brand</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {isUpdate && (
            <div className="form-group">
              <label className="form-control-label" htmlFor="oldbrand">
                Old Brand Title
              </label>
              <input
                type="text"
                className="form-control login-form-control"
                id="oldbrand"
                value={oldBrandName}
                disabled
              />
            </div>
          )}
          <div className="form-group">
            <label className="form-control-label" htmlFor="brand">
              {isUpdate && "New "}Brand Title
            </label>
            <input
              type="text"
              className="form-control login-form-control"
              id="brand"
              value={brandName}
              onChange={handleChange}
            />
            <p className="text-danger mb-0 font-weight-bold">{error}</p>
          </div>
        </Modal.Body>
        <Modal.Footer>
          <button
            className="btn btn-secondary btn-radius-0"
            onClick={closeModel}
          >
            Close
          </button>
          <button className="btn btn-pink-2 btn-radius-0" onClick={addCategory}>
            Save Changes
          </button>
        </Modal.Footer>
      </Modal>
    </>
  );
}
