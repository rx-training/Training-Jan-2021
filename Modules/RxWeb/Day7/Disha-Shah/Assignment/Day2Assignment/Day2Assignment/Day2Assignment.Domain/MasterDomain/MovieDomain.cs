...
namespace Day2Assignment.Domain.MasterModule
{
    public class MovieDomain : IMovieDomain
    {
        ...
        {
            var movieList = (object)Uow.Repository<Movie>().AllInclude(p => p.MovieGenres).Where(p => p.CertificationId == parameters.CertificationId);
            return Task.FromResult(movieList);
        }

        public Task<object> GetBy(Movie parameters)
        {
            var movieList = (object)this.Uow.Repository<Movie>().SingleOrDefault(m => m.MovieId == parameters.MovieId && m.CertificationId == parameters.CertificationId);
            return Task.FromResult(movieList);
        }
        ...
        public async Task AddAsync(MovieDTO entity)
        {
            ...
            movieEntity.CertificationId = entity.CertificationId;
        ...
        {
            var existingMovie = this.Uow.Repository<Movie>().SingleOrDefault(m => m.MovieId == entity.MovieId && m.CertificationId == entity.CertificationId);
            ...
            existingMovie.CertificationId = entity.CertificationId;
        ...
        {
            Movie movie = Uow.Repository<Movie>().SingleOrDefault(p => p.MovieId == parameters.MovieId && p.CertificationId == parameters.CertificationId);
