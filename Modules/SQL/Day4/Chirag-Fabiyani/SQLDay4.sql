...
SELECT DENSE_RANK() OVER(ORDER BY Salary DESC) AS SalaryRank,Salary FROM Employees;



----Select 4th Highest salary from employee table using ranking function----

SELECT * FROM (SELECT *, DENSE_RANK() OVER (ORDER BY SALARY DESC) AS SalaryRank FROM Employees) AS tbl WHERE SalaryRank=4;
...
SELECT DepartmentID,MAX(Salary) AS MaximumSalary FROM Employees GROUP BY (DepartmentID) ORDER BY MAX(Salary) ASC;



----Get department wise minimum salary from employee table order by salary ascending----

SELECT DepartmentID,MIN(Salary) AS MinimunSalary FROM Employees GROUP BY (DepartmentID) ORDER BY MIN(Salary) ASC;
...
SELECT * FROM 
(SELECT DepartmentID,SUM(Salary) AS TotalSalary FROM Employees GROUP BY DepartmentID) AS tbl2 
