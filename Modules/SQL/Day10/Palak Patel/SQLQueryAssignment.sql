...
INSERT INTO Employees1(EmployeeID,FirstName,LastName , Email, PhoneNumber, HireDate, JobId, Salary, CommissionPct, ManagerID, DepartmentID) VALUES
...
SET NOCOUNT ON
DECLARE @emp_salary MONEY, @hike MONEY, @emp_id INT

DECLARE hike_cursor CURSOR FOR
	SELECT Salary, EmployeeID FROM Employees1

OPEN hike_cursor

FETCH NEXT FROM hike_cursor INTO @emp_salary, @emp_id

WHILE @@FETCH_STATUS = 0
BEGIN
	UPDATE Employees1
	SET Salary = CASE
		WHEN @emp_salary BETWEEN 3000 AND 4000 THEN  @emp_salary + 500
		WHEN @emp_salary BETWEEN 4000 AND 5500 THEN  @emp_salary + 700
		WHEN @emp_salary BETWEEN 5500 AND 6500 THEN  @emp_salary + 900
		ELSE @emp_salary
	END WHERE EmployeeID=@emp_id
	FETCH NEXT FROM hike_cursor INTO @emp_salary, @emp_id
END
CLOSE hike_cursor
