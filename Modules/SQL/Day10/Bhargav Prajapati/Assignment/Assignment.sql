CREATE DATABASE AssignmentdbDay10
USE AssignmentdbDay10
...
DECLARE @Salary int,@empid int ,@temp int 

DECLARE Salaryupdate CURSOR  FOR SELECT Salary,EmployeeID FROM Employees

OPEN Salaryupdate

FETCH NEXT FROM Salaryupdate INTO @Salary,@empid 
...
BEGIN
	IF @Salary BETWEEN 3000 AND 4000
		BEGIN
			...
			UPDATE Employees SET Salary=@temp WHERE EmployeeID=@empid
		END
			IF @Salary BETWEEN 4000 AND 5000
			
				SET @temp=@Salary+7000
			UPDATE Employees SET Salary=@temp WHERE EmployeeID=@empid
			
		 IF @Salary BETWEEN 5500 AND 6500 
			BEGIN	
				SET @temp=@Salary+9000
			UPDATE Employees SET Salary=@temp WHERE EmployeeID=@empid
			END
		IF (@Salary>6500 OR @Salary<3000)
			BEGIN
				SET @temp=@Salary
				UPDATE Employees SET Salary=@temp WHERE EmployeeID=@empid
			END	
