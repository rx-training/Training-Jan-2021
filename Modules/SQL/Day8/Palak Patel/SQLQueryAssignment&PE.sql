...
--Therefore, you need to create a new clustered index to help with overall performance. Using the SSMS interface, 
--what steps would you use to create a new clustered index on the name column for the AdventureWorks database?
...
CREATE CLUSTERED INDEX IX_EID ON Employees (EmployeeID ASC)

CREATE CLUSTERED INDEX IX_PID ON PlanetsID (ID ASC)

CREATE NONCLUSTERED INDEX NIX_EMP ON Employees (FirstName, DepartmentID) 
