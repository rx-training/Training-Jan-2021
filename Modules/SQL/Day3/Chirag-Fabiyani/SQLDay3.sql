...
SELECT FirstName, DATALENGTH(FirstName) AS LengthofFName FROM Employees WHERE FirstName LIKE 'A%' OR FirstName LIKE 'J%' OR FirstName LIKE 'M%' ORDER BY FirstName;
...
SELECT FirstName,REPLICATE('$',10-DATALENGTH(LEFT(salary, 10)))+LEFT(salary, 10) AS SALARY FROM Employees;
...
SELECT FirstName,DATALENGTH(FirstName) AS LengthofFName FROM Employees WHERE CHARINDEX('c',FirstName)>2;



----Write a query to extract the last 4 character of PhoneNumber.----

SELECT RIGHT(PhoneNumber,4) AS RightFour FROM Employees;



----Write a query to update the portion of the PhoneNumber in the employees table, within the phone number the substring----
...
SELECT HireDate,GETDATE() CurrentDate, DATEDIFF(year,HireDate,GETDATE()) DiffDate FROM Employees;
...
SELECT SUBSTRING(CONVERT(VARCHAR, GETDATE(), 100), 13, 5)+' '+SUBSTRING(CONVERT(VARCHAR, GETDATE(), 100), 18, 2)+' '+DATENAME(month,GETDATE())+' '+DATENAME(day,GETDATE())+', '+DATENAME(year,GETDATE()) AS FormattedDate;
