CREATE DATABASE AssingmentDBDay3
USE AssingmentDBDay3
...
/*Write a query to display the FirstName and Salary for all employees. Format the salary to be 10
...
/*Write a query to display the employees with their code, first name, last name and hire date who hired either 
...
SELECT FirstName,LEN(FirstName) AS 'Length OF First Name' FROM Employees WHERE LastName LIKE '__c%'

/*Write a query to extract the last 4 character of PhoneNumber.*/
SELECT SUBSTRING(PhoneNumber,9,4) AS '4 Digit Phone Number' FROM Employees

/*Write a query to update the portion of the PhoneNumber in the employees table, within the
phone number the substring ‘124’ will be replaced by ‘999’ */
SELECT PhoneNumber,REPLACE(PhoneNumber,'124','999') AS 'Replaced Phone Number ' FROM Employees

/*Write a query to calculate the age in year*/
SELECT DATEDIFF(YEAR,'1999-12-11',GETDATE()) AS 'Age';

/*Write a query to get the distinct Mondays from HireDate in employees tables*/
SELECT *,DATENAME(WEEKDAY,HireDate) AS 'Distinct Monday' FROM Employees WHERE DATENAME(WEEKDAY,HireDate)='Monday'
...
SELECT FORMAT(GETDATE(),'HH:MM tt MMM, yyyy') AS 'Time In Formate'
