...
SELECT FirstName, Length = LEN(FirstName) FROM Employees WHERE FirstName LIKE '[AJM]%' ORDER BY FirstName
...
SELECT FirstName, SALARY='$' + LEFT(salary, 10) FROM employees
...
SELECT EmployeeID, Name=(FirstName+ ' ' +LastName), HireDate FROM Employees WHERE DAY(HireDate)=07 OR MONTH(HireDate) = 07 ORDER BY DAY(HireDate)

--Write a query to display the length of first name for employees where last name contains character ‘c’ after 2nd position.
SELECT LEN(FirstName) AS 'Length of name', LastName, FirstName FROM Employees WHERE	LastName LIKE '_c%'
...
UPDATE Employees SET PhoneNumber = STUFF(PhoneNumber, 5, 3, '999') WHERE PhoneNumber LIKE '%.124.%'
SELECT PhoneNumber FROM Employees

--Write a query to calculate the age in year.
SELECT AGE=(YEAR(SYSDATETIME ( ))-YEAR('1998-02-12'))

--Write a query to get the distinct Mondays from HireDate in employees tables.
SELECT FirstName, HireDate FROM Employees WHERE DATENAME(WEEKDAY, HireDate)='Monday'
...
SELECT Date=FORMAT(SYSDATETIME(), 'hh:mm tt')+ ' ' + DATENAME(MONTH, SYSDATETIME ( )) + ' ' + DATENAME(DAY, SYSDATETIME ( ))+ ', ' + DATENAME(YEAR, SYSDATETIME ( ));

--Write a query to get the FirstName, LastName who joined in the month of June.
SELECT Name=FirstName + ' ' +LastName, HireDate FROM Employees WHERE MONTH(HireDate) = 06

--Write a query to get first name, hire date and experience of the employees.
SELECT FirstName AS Name, HireDate, Experience = (YEAR(SYSDATETIME ( ))-YEAR(HireDate)) FROM Employees
