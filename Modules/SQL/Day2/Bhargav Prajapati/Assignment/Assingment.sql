CREATE DATABASE  Assignmentdb_Day2
...
/*Write a SQL statement to change the Email and CommissionPct column of employees table with ‘not available’ and 0.10 
for all employees.*/
...
UPDATE Employees SET Email='not available' WHERE DepartmentID =80 AND CommissionPct<.2
SELECT * FROM Employees
...
UPDATE Employees SET JobId='SH_CLERK' WHERE EmployeeID=118 AND DepartmentID=30 AND JobId NOT In('SH')
SELECT * FROM Employees
/*Write a SQL statement to increase the salary of employees under the department 40, 90 and 110 according to the company 
rules that, salary will be increased by 25% for the department 40, 15% for department 90 and 10% for the department 110 and 
the rest of the departments will remain same.*/
UPDATE Employees SET Salary= Salary+ CASE
				WHEN DepartmentID=40 THEN Salary*.25
				WHEN DepartmentID=90 THEN Salary*.15
				WHEN DepartmentID=110 THEN Salary*.10
				ELSE Salary*0
				END;
...
SELECT EmployeeID,'Name OF Employee'= FirstName +' ' +' '+ LastName ,Salary FROM Employees ORDER BY Salary
