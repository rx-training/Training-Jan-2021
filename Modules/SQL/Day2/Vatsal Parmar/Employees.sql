...
UPDATE Employees Set Email = 'not available',CommissionPct = '0.10' WHERE DepartmentID = '80' AND CommissionPct < '0.20' ;
...
/* 6 - Write a SQL statement to change job ID of employee which ID is 118, to SH_CLERK if the employee belongs to department, which ID is 30 and the existing job ID does not start with SH. */

UPDATE Employees Set JobId = 'SH_CLERK' WHERE EmployeeId = 118 AND DepartmentId = '30' AND JobId  NOT LIKE '^SH' ;

/* 7 - Write a SQL statement to increase the salary of employees under the department 40, 90 and 110 according to the company rules that, salary will be increased by 25% for the department 40, 15% for department 90 and 10% for the department 110 and the rest of the departments will remain same.*/
...
SELECT FirstName,LastName,'PF'=Salary*0.12 FROM Employees;
