...
WITH RankEmp(EmployeeID, FirstName, LastName, Salary,Rank,DenseRank,RowNumber) AS
(
...
--derived table
SELECT * FROM 
	(SELECT *, DENSE_RANK() OVER (ORDER BY Salary DESC) AS 'RowNumber' FROM Employees) AS Result 
	WHERE RowNumber = 4
...
(
	SELECT *, DENSE_RANK() OVER (ORDER BY Salary DESC) AS 'RowNumber' FROM Employees
)

SELECT * FROM HighestSalary WHERE RowNumber = 4
...
---------------------------CTE------------------------
WITH MYCTE(DepartmentID,TotalSalary) AS
(
...
/*Get department, total salary with respect to a department from employee table order
by total salary descending*/

---------------------------CTE-----------------------

WITH SalaryDesc(DepartmentID,TotalSalary) AS
(
	SELECT DepartmentID,SUM(SALARY) FROM Employees
	GROUP BY (DepartmentID)
)

SELECT * FROM SalaryDesc ORDER BY TotalSalary DESC
...
SELECT * FROM MyCTE ORDER BY TotalSalary DESC
