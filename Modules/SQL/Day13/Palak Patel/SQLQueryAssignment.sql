...
GO
ALTER FUNCTION dbo.ComputePF (@BasicSalary INT)
RETURNS FLOAT
AS
BEGIN
...
GO
ALTER FUNCTION dbo.ComputePT (@MontlyEarning INT)
RETURNS VARCHAR(30)
AS
BEGIN
	DECLARE @PT VARCHAR(30)
	SET @PT = CASE
		WHEN @MontlyEarning < 5999 THEN 'Nil'
		WHEN @MontlyEarning BETWEEN 6000 AND 8999 THEN 'Rs. 80/Month'
		WHEN @MontlyEarning BETWEEN 9000 AND 11999 THEN 'Rs. 150/Month'
		WHEN @MontlyEarning > 12000 THEN 'Rs. 200/Month'
	END
	RETURN @PT
END
GO

DECLARE @WAGE VARCHAR(30)
SET @WAGE = dbo.ComputePT(10925)
PRINT @WAGE

--Create a table valued function which will accept JobTitle which will return new table set based on jobtitle
GO
CREATE FUNCTION dbo.q3JobTitle (@JobTitle VARCHAR(30))
RETURNS TABLE
AS
RETURN(SELECT * FROM Employees WHERE JobId = @JobTitle)
GO

SELECT * FROM dbo.q3JobTitle('SA_REP')